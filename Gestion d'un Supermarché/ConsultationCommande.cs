...
namespace Gestion_d_un_Supermarché
{
    public partial class ConsultationCommande : Form
    {
        ...
        void EnTete()
        {
            string[] nc = { "Numéro", "CIN du client", "ID d'employé", "Prix total", "Date de commande" };
            for (int i = 0; i < 5; i++)
            {
        ...
        private void ConsultationCommande_Load(object sender, EventArgs e)
        {
            ModeCS();
            dataGridView1.DataSource = outils.RemplirDataGridView("SELECT * FROM Commande");
            EnTete();
        }

        private void ConsultationCommande_FormClosing(object sender, FormClosingEventArgs e)
        {
            ...
            if (openFileDialog1.ShowDialog() == DialogResult.OK)
            {
                try
                {
                    ...
                    if (openFileDialog1.ShowDialog() == DialogResult.OK)
                    {
                        ...
                        dataGridView1.DataSource = outils.ImporterXML("Commande", fichierXML, fichierXSD);
                        EnTete();
                    }
                }
                catch
                {
                    MessageBox.Show("Veuillez choisir des fichiers XSD et XML valides.", "Importation des commandes", MessageBoxButtons.OK, MessageBoxIcon.Error);
                }
            }
        }
    }
